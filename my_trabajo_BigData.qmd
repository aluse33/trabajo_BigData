...
date: 2024-01-23                           #--
categories: [trabajo BigData, economía, programación, crecimiento económico, universidad, CO2, uranio, renta pc]   #--
image: ""
...
## 1. Cerrar las facultades de Humanidades (y la mitad de Ciencias Sociales casi que también).

No quiero que el lector se enfade todavía, no le guardo ningún tipo de animadversión a esta rama del conocimiento, de hecho, un servidor, en primero de carrera, estuvo muy cerca de abandonar Economía y matricularse en Historia, esta decisión casi se hizo efectiva, fundamentalmente, por la cantidad de asignaturas de mierda y de escaso valor (ejem, Derecho Mercantil, Dirección de Empresas, Contabilidad Financiera, etc.) que a uno le hacen estudiar para formarse como economista (??). Además de esto, a un servidor también le apasiona la Filosofía, como dato anecdótico, muy probablemente haya consumido más horas de contenido de Ernesto Castro en YouTube que de cualquier otro creador de contenido, esto toma relevancia cuando uno sabe que sus vídeos tienen una duración aproximada, en promedio, de dos horas, y que son lecciones de Ética, Estética o de Historia de la Filosofía en la Universidad Complutense de Madrid.
...
Pero, como habrá quedado claro, esto no es más que un **hobby**, y el momento en el que uno se matricula en uno de estos grados, pasa a ser un hobby para ricos, no por el coste económico de esta decisión, sino por el coste de oportunidad. En un país funcional en el que el gasto público en educación universitaria siguiese unos criterios de optimización, el coste de estos grados seria altísimo, es decir, no estaría subvencionado en su práctica totalidad como sí sucede ahora. Más o menos, sería similar a lo que hoy en día puede ser aprender a tocar el piano pagando las tasas de una Escuela de Música o de un Conservatorio.

En este sentido, es interesante analizar los salarios, el desempleo, el número de titulados y todas estas variables a la hora de estudiar lo que sucede con la demanda laboral de los graduados en estas ramas. Para ello, tenemos la [Encuesta de Inserción Laboral del año 2019 que el INE](https://www.ine.es/ss/Satellite?L=es_ES&c=INESeccion_C&cid=1259948984778&p=%2F&pagename=ProductosYServicios%2FPYSLayout&param1=PYSDetalle&param3=1259924822888) pone a nuestra disposición. 
...
df_paro <- paro %>% 
  na.omit() %>% 
  rename(Titulación = `Inserción laboral de titulados universitarios`, Desempleo = ...2) 

#- los datos de tasas de paro están en character, los pasamos a numeric
...
tabla_paro <- gt(df_paro) %>%
  gtExtras::gt_plt_dot(column = "Desempleo", category_column = `Titulación`,  
                       max_value = NULL, palette = "wesanderson::IsleofDogs1") %>%
  gtExtras::gt_theme_guardian() %>%
  cols_width(`Titulación` ~ px(400), 2 ~ px(150)) %>% 
  tab_header(title = "Top 15 titulaciones con mayor tasa de paro.",
             subtitle = "Año 2019, datos obtenidos del INE.")
#- tabla_paro
```
## Tabla
```{r}
...
De hecho, los datos de desempleo todavía pueden ocultarnos información: al fin y al cabo, quizá, si estudias Filosfía no tienes trabajo como filósofo, pero sí en algo totalmente ajeno a ello y acabas trabajando ahí. Podemos analizar, también, el nivel óptimo de estudios demandado para el puesto de trabajo en el que se encuentra un titulado en x grado, de manera que si, por ejemplo, el 35% de los titulados en Filosofía trabajan en algo para lo que no era necesario el grado y, según los datos, con la ESO les bastaba, es evidente que ese algo no tiene relación alguna con el mundo de la Filosofía.
...
nvl <- nvl %>% 
  slice(6, 9:110) %>% 
  rename(`Grados y nivel relativo óptimo de estudios por empleo` = `Graduados universitarios. Cifras relativas`, Doctorado = ...2, `Máster` = ...3, `Título` = ...4, `Grado superior` = ...5, `Grado Medio` = ...6, `Grado básico` = ...7, ESO = ...8) %>% 
  arrange(`Título`) %>% 
  na.omit()
...
tabla_nvl <- DT::datatable(
  ...
  caption = htmltools::tags$caption(
    ...
    '', htmltools::em("Nivel óptimo de estudios en función del empleo por titulación del graduado")),
  filter = "top",
...
Ahora podemos ver el porcentaje de titulados que podrían trabajar en su actual puesto de trabajo sin haber estudiado la carrera que han estudiado:
...
nvl <- nvl %>% 
  ...
  mutate(`Porcentaje de titulados que podrían trabajar sin el título` = rowSums(select(., `Grado superior`:ESO), na.rm = TRUE)) %>% 
  mutate(`Porcentaje de titulados que necesitan el título en su puesto` = rowSums(select(., Doctorado:`Título`), na.rm = TRUE)) %>% 
  arrange(desc(`Porcentaje de titulados que podrían trabajar sin el título`)) %>% 
  top_n(15, wt = `Porcentaje de titulados que podrían trabajar sin el título`) %>% 
  select(`Grados y nivel relativo óptimo de estudios por empleo`, `Porcentaje de titulados que podrían trabajar sin el título`)

table_nvl <- gt(nvl) %>%
  gtExtras::gt_plt_dot(column = "Porcentaje de titulados que podrían trabajar sin el título", category_column = `Grados y nivel relativo óptimo de estudios por empleo`,  
                       max_value = NULL, palette = "wesanderson::Darjeeling1") %>%
  gtExtras::gt_theme_espn() %>%
  cols_width(`Grados y nivel relativo óptimo de estudios por empleo` ~ px(400), 2 ~ px(150)) %>% 
  tab_header(title = "Top 15 titulaciones que te llevarán a trabajar en cualquier cosa menos en algo relacionado con lo que has estudiado.",
             subtitle = "Año 2019, datos obtenidos del INE.")
#- table_nvl
```
## Tabla
...
tabla_sueldos <- DT::datatable(
  ...
  caption = htmltools::tags$caption(
    style = 'caption-side: top; text-align: center;',
    '', htmltools::em("Sueldos en el primer empleo en función de la titulación")),
  filter = "top",
  class = "cell-border stripe",
  extensions = 'Buttons', 
...
El SMI es, actualmente, superior a 1000€, por lo que, previsiblemente, todo aquel que cobre menos de 1000€ será porque:

1. Está subempleado (i.e. no trabaja a jornada completa, evidentemente, alguno querrá que esto sea así, pero la mayor parte del subempleo es involuntario).

2. Trabajará como becario cobrando menos del SMI.
...
sueldos <- sueldos %>% 
  mutate(across(-1, as.numeric)) %>% 
  mutate(`Menos de 1000€ netos al mes` = `Menos de 700 euros` + `De 700 a 999 euros`) %>% 
  select(`Graduados universitarios. Cifras relativas`, `Menos de 1000€ netos al mes`) %>% 
  arrange(desc(`Menos de 1000€ netos al mes`)) %>% 
  top_n(15, wt = `Menos de 1000€ netos al mes`)

table_sueldos <- gt(sueldos) %>%
  gtExtras::gt_plt_dot(column = "Menos de 1000€ netos al mes", category_column = `Graduados universitarios. Cifras relativas`, max_value = NULL, palette = "wesanderson::BottleRocket1") %>%
  gtExtras::gt_theme_nytimes() %>%
  cols_width(`Graduados universitarios. Cifras relativas` ~ px(400), 2 ~ px(150)) %>% 
  tab_header(title = "Top 15 titulaciones con las que es más probable ganar menos de 1000€ al mes.", subtitle = "Año 2019, datos obtenidos del INE.")
...
Estos temas están a la orden del día dentro del debate público, de hecho, hoy al leer Twitter mientras desayunaba, me he encontrado con una serie de tweet que ilustran esta situación y me parece que nos vienen al pelo, por lo que los adjunto por aquí abajo:
...
Pues eso, parece que todo está dicho. Eso sí, un dato algo más optimista y con el que nos podríamos plantear no cerrar todas las facultades de Humanidades y transformarlas en facultades de Ingeniería, sería el del número de titulados en España en los diferentes grados, vemos que, por fortuna, la mayor parte de los egresados universitarios estudian carreras diferentes a las ya vistas:
...
#- quitamos las categorías y nos quedamos con las titulaciones

titulados <- titulados %>% 
  na.exclude() %>% 
  rename(Titulaciones = `Inserción laboral de titulados universitarios`, Graduados = ...2)
...
tabla_titulados <- gt(titulados) %>% 
  gtExtras::gt_theme_nytimes() %>% 
  tab_header(title = "Graduados por titulación en 2019.",
...
Si queremos verlo en un mapa resulta todavía más evidente, cuanto más oscuro más culpable (no penséis mal), y como podemos ver, son los que pensábamos: 
...
La solución es mucho más sencilla, traer de vuelta los [maravedíes]( https://en.wikipedia.org/wiki/Maraved%C3%AD) y respaldar su valor con reservas de uranio, algo así como el [Patrón Oro](https://es.wikipedia.org/wiki/Patr%C3%B3n_oro), pero un poco más moderno y con una curva de oferta mucho más “ideal” para nuestra moneda. La idea básica es la siguiente (permítanme hablar un poco de física):
...
$$\frac{dN}{dt} = -\lambda N$$
Siendo $\frac{dN}{dt}$ la tasa de variación del número de átomos con respecto al tiempo, $N$ la cantidad de átomos en un momento concreto y $-\lambda$ la constante de desintegración del uranio-238.
...
Habiendo reordenado los factores podemos integrar y obtener la solución general de nuestra EDO:
$$\int \frac{1}{N} \, dN = -\lambda \int \, dt$$
De modo que tenemos:

$$\ln(N) = -t\lambda + C$$
Finalmente, aplicando exponenciales y reordenando tenemos que la solución general es:
